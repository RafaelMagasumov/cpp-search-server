...
public:
	...
	std::vector<Document> FindTopDocuments(std::execution::sequenced_policy policy, std::string_view raw_query,
	...
	std::vector<Document> FindTopDocuments(std::execution::parallel_policy polity, std::string_view raw_query,
	...
	std::tuple<std::vector<std::string_view>, DocumentStatus> MatchDocument(std::execution::parallel_policy polity, std::string_view raw_query,

		int document_id) const;


	std::tuple<std::vector<std::string_view>, DocumentStatus> MatchDocument(std::execution::sequenced_policy polity, std::string_view raw_query,
...
private:
	...
	std::deque<std::string> storage; // типа буфера для ссылки string_view
	...
	const std::set<std::string, std::less<>> stop_words_; // новые стоп-слова с поддержкой string_view
...
std::vector<Document> SearchServer::FindTopDocuments(std::execution::sequenced_policy polity, std::string_view raw_query,
...
// парараллельная версия 

template <typename DocumentPredicate>

std::vector<Document> SearchServer::FindTopDocuments(std::execution::parallel_policy polity, std::string_view raw_query,
...
// FindAllDocument обычная версия 
...
// FindALlDocument последовательеая версия 
template <typename DocumentPredicate>
...
// FindAllDocument парарллееая версия.
...
std::vector<Document> SearchServer::FindAllDocuments(std::execution::parallel_policy policy, const Query& query,
	...
	DocumentPredicate document_predicate) const {
	//LOG_DURATION("Find ALl Documents");
